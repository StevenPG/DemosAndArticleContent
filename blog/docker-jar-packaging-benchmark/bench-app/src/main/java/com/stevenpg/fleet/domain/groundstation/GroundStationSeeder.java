package com.stevenpg.fleet.domain.groundstation;

import com.stevenpg.fleet.seed.SeedContributor;
import java.util.ArrayList;
import java.util.List;
import org.springframework.stereotype.Component;
import org.springframework.transaction.annotation.Transactional;

@Component
public class GroundStationSeeder implements SeedContributor {

    private static final String[] REGIONS = {"NAMER", "EMEA", "APAC", "LATAM"};

    private final GroundStationRepository repository;

    public GroundStationSeeder(GroundStationRepository repository) {
        this.repository = repository;
    }

    @Override
    public String domain() {
        return "groundstation";
    }

    @Override
    @Transactional
    public int seed(int rows) {
        List<GroundStation> batch = new ArrayList<>(rows);
        for (int i = 0; i < rows; i++) {
            GroundStation entity = new GroundStation();
            entity.setCode("SITE-%04d".formatted(i + 24 * 1000));
            entity.setName("GroundStation unit %d".formatted(i));
            entity.setStatus(GroundStationStatus.values()[i % GroundStationStatus.values().length]);
            entity.setRegion(REGIONS[i % REGIONS.length]);
            entity.setSiteCode("SITE%05d".formatted(i * 7 + 24));
            entity.setMassKg((i + 1) * 13.5d);
            entity.setCycles(i * 17);
            entity.setActive(i % 5 != 0);
            batch.add(entity);
        }
        return repository.saveAll(batch).size();
    }
}
