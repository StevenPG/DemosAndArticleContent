...
CREATE TABLE sensor_readings (
    -- UUIDv7: time-ordered, so the primary key is physically correlated with
    -- the heap instead of scattering across every leaf page the way random v4
    -- keys do.
    --
    -- Postgres 18 provides uuidv7() natively, and for a table whose rows are
    -- born in the database that is the right way to get one. It deliberately
    -- is NOT used here: the id identifies the event across Kafka, so it must
    -- exist at event-creation time in the producer, long before any row
    -- reaches Postgres. A DEFAULT would never fire either, since COPY always
    -- supplies the column. Generating it in two places would be worse than
    -- generating it in the one place that owns the event's identity.
    id          uuid             NOT NULL,
